...
function createFilterListTemplate(filters) {
  return filters.reduce((acc, filter, i) => {
    const filterItem = `<div class="trip-filters__filter">
        <input id="filter-${filter.id}" class="trip-filters__filter-input  visually-hidden" type="radio" name="trip-filter" value="${filter.type}" ${i === 0 ? 'checked' : ''} ${filter.count === 0 ? 'disabled' : ''}>
