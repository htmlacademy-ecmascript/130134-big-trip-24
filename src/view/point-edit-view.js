...
function createPointTypesTemplate(type, offers, isDisabled) {
  return offers.reduce((acc, pointType) => {
    const pointItem = `<div class="event__type-item">
        <input id="event-type-${pointType.type}-1" class="event__type-input  visually-hidden" type="radio" name="event-type" value="${pointType.type}" ${pointType.type === type ? 'checked' : ''} ${isDisabled ? 'disabled' : ''}>
...
function createControlButtonsTemplate(point) {
  return typeof point.id !== 'undefined'
...
function createOffersList(offers, pointOffers, isDisabled) {
  return offers.reduce((acc, offer) => {
    const offerItem = `<div class="event__offer-selector">
              <input class="event__offer-checkbox  visually-hidden" id="event-offer-${offer.id}-1" data-id="${offer.id}" type="checkbox" name="event-offer-${offer.id}" ${pointOffers.includes(offer.id) ? 'checked' : ''} ${isDisabled ? 'disabled' : ''}>
...
function createOffersTemplate(offers, point) {
  ...
  return `<section class="event__section  event__section--offers">
            ...
            <div class="event__available-offers">
              ${createOffersList(pointOffersItems.offers, point.offers, point.isDisabled)}
            </div>
          </section>`;
}
...
function createEditPointTemplate(point, offers, destinations) {
  ...
  const pointTypesTemplate = createPointTypesTemplate(point.type, offers, point.isDisabled);
  const destinationsOptionsTemplate = createDestinationsListTemplate(destinations);
  const pointControlsTemplate = createControlButtonsTemplate(point);
      ...
      <form class="event event--edit" action="#" method="post">
        <header class="event__header">
          ...
          <div class="event__field-group  event__field-group--time">
            ...
            <input class="event__input  event__input--time" id="event-start-time-1" type="text" name="event-start-time" value="${dateFrom}" ${point.isDisabled ? 'disabled' : ''}>
            &mdash;
            <label class="visually-hidden" for="event-end-time-1">To</label>
            <input class="event__input  event__input--time" id="event-end-time-1" type="text" name="event-end-time" value="${dateTo}" ${point.isDisabled ? 'disabled' : ''}>
          </div>
          ...
          <div class="event__field-group  event__field-group--price">
            ...
            <input class="event__input  event__input--price" id="event-price-1" type="number" max="100000" name="event-price" value="${point.basePrice}" ${point.isDisabled ? 'disabled' : ''}>
          </div>
